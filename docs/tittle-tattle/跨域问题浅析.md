...
head:
  - - meta
    - name: description
      content: 在 Web 开发过程中，跨域的出现源于浏览器的同源策略。解决跨域问题最好的方式是采用 CORS 协议。同源策略是什么？CORS为什么能解决跨域问题？工作机制如何？前后端开发者在各自岗位中如何共同解决跨域问题？本文将带你解答以上问题。
  - - meta
    - name: keywords
      content: 跨域, CORS, 同源策略, 前端, Web, JSONP, DevServer
---
