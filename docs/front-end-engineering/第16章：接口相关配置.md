...
// errorConfig 的属性值是一个对象，key为statueCode，value为{default: boolean, message: string|boolean}类型，用于控制对应错误是否需要统一处理
...
在响应拦截器中，可以对响应数据进行规范化处理，例如将响应数据中的 data 字段提取出来，或者将响应数据中的错误信息转换为统一的格式。
